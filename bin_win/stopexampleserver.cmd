...
IF DEFINED USERNAME (
    ...
    ECHO Administrator ID and password is not set in jeus-example.properties.cmd file.
    GOTO END	
)

"%JAVA_HOME%\bin\java" -classpath %BOOTSTRAP_CLASSPATH% %TOOL_OPTION% -Djeus.home="%JEUS_HOME%" -Djava.naming.factory.initial=jeus.jndi.JEUSContextFactory -Djava.naming.factory.url.pkgs=jeus.jndi.jns.url -Djeus.baseport=%JEUS_BASEPORT% -Djava.util.logging.config.file="%JEUS_HOME%\bin\logging.properties" %BOOTSTRAPPER%  jeus.server.manager.JeusCommander example %BOOT_PARAMETER% jeusexit
